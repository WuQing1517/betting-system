...
def get_competition_full(competition_id):
    ...
    for m in matches:
        ...
        matches_data.append({'id': m.id, 'match_code': m.match_code, 'week_number': m.week_number, 'day_number': m.day_number, 'match_number': m.match_number, 'home_team': m.home_team, 'away_team': m.away_team, 'status': m.status, 'questions': questions_data})
    return jsonify({'id': competition.id, 'name': competition.name, 'year': competition.year, 'season': competition.season, 'status': competition.status, 'matches': matches_data})
